...
function TextStyleSelect({ editor }: { editor: Editor }) {
  const activeHeading = HEADING_OPTIONS.find((opt) =>
    opt.level ? editor.isActive('heading', { level: opt.level }) : editor.isActive('paragraph'),
  );
  ...
  return (
    ...
    >
      ...
      <ToolbarButton
        ...
        title="Insert table"
      />
      <ToolbarButton onClick={insertImage} icon={<ImagePlus size={16} />} title="Insert image" />
