...
vi.mock("@worknest/ui", () => ({
  Button: ({
    ...
    React.createElement("button", { onClick, className, "data-variant": variant, ...rest }, children),
...
describe("IssueListTable", () => {
  ...
  it("checkbox selection toggles via the checkbox control", () => {
    const issues = [
      makeIssue({ id: "issue-sel", sequenceId: 5, title: "Select me" }),
    ];
  ...
  it("shows due date with warning color when overdue", () => {
    ...
    const overdueSpan = Array.from(spans).find(
  ...
  it("does not call onRowClick for temp issues", () => {
    const onRowClick = vi.fn();
    const issues = [
      makeIssue({ id: "temp-123", sequenceId: 0, title: "Temp issue" }),
    ];
  ...
  it("shows '...' as key for temp issues", () => {
    const issues = [
      makeIssue({ id: "temp-abc", sequenceId: 0, title: "Creating..." }),
    ];
