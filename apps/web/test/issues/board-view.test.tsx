...
describe("KanbanBoard", () => {
  ...
  it("card title has line-clamp-2 class for truncation", () => {
    const issues = [
      makeIssue({
        id: "i-trunc",
        sequenceId: 7,
        title: "A very long title that should be truncated after two lines to keep the card compact",
        statusId: "status-open",
      }),
    ];
  ...
  it("due date shows only when within 3 days or overdue", () => {
    ...
    const hasDueDate = Array.from(dueElements).some(
      (el) => el.className?.includes("text-orange-500"),
    );
    ...
    const farHasDueDate = Array.from(farDueElements).some(
      (el) => el.className?.includes("text-orange-500") || el.className?.includes("text-destructive"),
    );
