...
describe("WebSocket manager", () => {
  ...
  describe("subscribe", () => {
    ...
    it("re-subscribes to channels after reconnect", () => {
      ...
      const sendCalls = secondInstance.send.mock.calls.map(
  ...
  describe("auto-reconnect", () => {
    ...
    it("does not reconnect after explicit disconnect", () => {
      ...
      const connectingInstances = MockWebSocket.instances.filter(
  ...
  describe("message queue", () => {
    it("queues subscribe messages when disconnected", () => {
      ...
      const sendCalls = MockWebSocket.lastInstance!.send.mock.calls.map(
        (call: unknown[]) => JSON.parse(call[0] as string),
      );
