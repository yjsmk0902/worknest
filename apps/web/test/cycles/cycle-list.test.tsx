...
describe('CycleStatusBadge', () => {
  ...
  it('applies status-specific class names to badges', () => {
    const { rerender } = render(React.createElement(CycleStatusBadge, { status: 'draft' }));
...
describe('CycleProgressBar', () => {
  it('renders empty bar when progress is undefined', () => {
    const { container } = render(React.createElement(CycleProgressBar, { progress: undefined }));
  ...
  it('renders correct number of segments based on non-zero categories', () => {
    ...
    const { container } = render(React.createElement(CycleProgressBar, { progress }));
...
describe('CycleProgressText', () => {
  it('renders null when progress is undefined', () => {
    const { container } = render(React.createElement(CycleProgressText, { progress: undefined }));
...
describe('CycleEmptyState', () => {
  it('renders empty state message and CTA button', () => {
    ...
    render(React.createElement(CycleEmptyState, { onCreateClick }));

    expect(screen.getByText('아직 사이클이 없습니다')).toBeDefined();
    expect(
      screen.getByText('사이클을 만들어 이슈를 그룹화하고 진행 상황을 추적하세요'),
    ).toBeDefined();
  ...
  it('renders the refresh icon', () => {
    render(React.createElement(CycleEmptyState, { onCreateClick: vi.fn() }));
