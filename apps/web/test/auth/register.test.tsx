...
vi.mock("@worknest/ui", () => ({
  Button: ({ children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { size?: string }) => {
    const { size, ...htmlProps } = props as { size?: string } & React.ButtonHTMLAttributes<HTMLButtonElement>;
    return React.createElement("button", htmlProps, children);
  },
  Input: React.forwardRef(
    (props: React.InputHTMLAttributes<HTMLInputElement> & { error?: boolean }, ref: React.Ref<HTMLInputElement>) => {
      const { error, ...htmlProps } = props;
      return React.createElement("input", { ...htmlProps, ref, "data-error": error });
    },
  ),
  Label: ({ children, ...props }: React.LabelHTMLAttributes<HTMLLabelElement> & { error?: boolean }) => {
    const { error, ...htmlProps } = props as { error?: boolean } & React.LabelHTMLAttributes<HTMLLabelElement>;
